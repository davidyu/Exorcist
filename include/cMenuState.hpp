    ...
    public:
        ...
        void HandleInput(CORE::cGame* game) {
            ...
            if (input.OnKeyDown(SDLK_RETURN)) {
                game->GetStateManager().PopStateUsingTransition(game->transition_factory.CreateObject("transFade"));
            }
        }
        ...
        void Render(CORE::cGame* game, float percent_tick) {
            GFX::RenderFullViewportTexture(*m_Tex, game->GetSDLState().window_w, game->GetSDLState().window_h);
        }
